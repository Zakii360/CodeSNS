...
window.sendVerificationCode = async function() {
    ...
    try {
        const { data, error } = await sb.functions.invoke('send-verification', { body: { email, userId: currentUser.id }, method: 'POST' });
        if (error) throw error;
        document.getElementById('verify-step-1').style.display = 'none'; document.getElementById('verify-step-2').style.display = 'block';
    } catch (err) {
        alert('Failed to send code: ' + (err.message || err.context?.message || 'Unknown error. Check Edge Function logs.'));
